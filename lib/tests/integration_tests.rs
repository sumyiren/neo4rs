...
mod integration_tests {
    ...
    async fn setup_session(max_connections: usize) -> Session {
        ...
        session.execute("MATCH (n) DETACH DELETE n".to_string(), None, None).await.unwrap();
        session
    }
    ...
    async fn test_session_execute() {
        ...
        let _result = session.execute("CREATE (n: Person {name:'apple'}) RETURN n".to_string(), None, None).await.unwrap();
    }
